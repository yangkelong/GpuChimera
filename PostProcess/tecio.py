...
import numpy as np
import sys
from DubheGpuSolver.setting.profile import ProjectDir


def loadLibrary():
    if sys.platform.startswith('win'):
        return ctypes.cdll.LoadLibrary(ProjectDir + "libs\\tecio.dll")
    elif sys.platform.startswith('linux'):
        return ctypes.cdll.LoadLibrary(ProjectDir + "libs/tecio.so")
...
def open_file(
    ...
    debug = ctypes.c_int32(1 if debug else 0)  # 0=No, 1=Yes
...
def zone_write_arr(arr, VIsDouble=True):
    if VIsDouble:
        isdouble = ctypes.byref(ctypes.c_int32(1))
    else:
        arr = arr.astype(np.float32)
        isdouble = ctypes.byref(ctypes.c_int32(0))
    ret = tecio.tecdat142(ctypes.byref(ctypes.c_int32(arr.shape[0])), arr.ctypes, isdouble)
    if ret != 0:
        raise Exception("zone_write_float_values Error")
